...
const DeliveredPO = ({ navigation }) => {
    ...
    const [deliveredPoList, setDeliveredPoList] = useState([])
    ...
    async function getPurchaseOrderList(pageNum) {
        ...
        if (temp.status == 'success') {
            setDeliveredPoList(deliveredPoList.concat(temp.data.purchaseOrder))
        }
        setIsLoading(false)
    }
    ...
    return (
        <SafeAreaView style={{ height: height, flex: 1 }}>
            {deliveredPoList.length != 0 ? <FlatList
                showsVerticalScrollIndicator={false}
                data={deliveredPoList}
                renderItem={({ item }) => {
