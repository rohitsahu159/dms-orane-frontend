...
const PartialPO = ({ navigation }) => {
    const dispatch = useDispatch()
    const [pageNumber, setPageNumber] = useState(0)
    const [partialPoList, setPartialPoList] = useState([])
    ...
    async function getPurchaseOrderList(pageNum) {
        ...
        if (temp.status == 'success') {
            setPartialPoList(partialPoList.concat(temp.data.purchaseOrder))
        }
        setIsLoading(false)
    }
    ...
    return (
        <SafeAreaView style={{ height: height, flex: 1 }}>
            {partialPoList.length != 0 ? <FlatList
