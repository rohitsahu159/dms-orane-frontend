...
const PendingForAprovalPO = ({ navigation }) => {
    const dispatch = useDispatch()
    const [pageNumber, setPageNumber] = useState(0)
    const [pendingForAprovalPoList, setPendingForAprovalPoList] = useState([])
    ...
    async function getPurchaseOrderList(pageNum) {
        ...
        if (temp.status == 'success') {
            setPendingForAprovalPoList(pendingForAprovalPoList.concat(temp.data.purchaseOrder))
        }
        setIsLoading(false)
    }
    ...
    return (
        <SafeAreaView style={{ height: height, flex: 1 }}>
            {pendingForAprovalPoList.length != 0 ? <FlatList
                showsVerticalScrollIndicator={false}
                data={pendingForAprovalPoList}
                renderItem={({ item }) => {
...
export default PendingForAprovalPO
