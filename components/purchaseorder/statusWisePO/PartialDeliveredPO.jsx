...
const PartialDeliveredPO = ({ navigation }) => {
    ...
    const [partialDeliveredPoList, setPartialDeliveredPoList] = useState([])
    ...
    async function getPurchaseOrderList(pageNum) {
        ...
        if (temp.status == 'success') {
            setPartialDeliveredPoList(partialDeliveredPoList.concat(temp.data.purchaseOrder))
        }
        setIsLoading(false)
    }
    ...
    return (
        <SafeAreaView style={{ height: height, flex: 1 }}>
            {partialDeliveredPoList.length != 0 ? <FlatList
                showsVerticalScrollIndicator={false}
                data={partialDeliveredPoList}
                renderItem={({ item }) => {
...
export default PartialDeliveredPO
