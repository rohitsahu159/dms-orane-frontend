...
const InProgressPO = ({ navigation }) => {
    ...
    const [inProgressPoList, setInProgressPoList] = useState([])
    ...
    async function getPurchaseOrderList(pageNum) {
        ...
        if (temp.status == 'success') {
            setInProgressPoList(inProgressPoList.concat(temp.data.purchaseOrder))
        }
        setIsLoading(false)
    }
    ...
    return (
        <SafeAreaView style={{ height: height, flex: 1 }}>
            {inProgressPoList.length != 0 ? <FlatList
                showsVerticalScrollIndicator={false}
                data={inProgressPoList}
                renderItem={({ item }) => {
